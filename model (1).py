...
if platform == 'android':
    ...
    class TensorFlowModel():
        def load(model_filename, num_threads=None):
        ...
        def pred(x):
            ...
            output = TensorBuffer.createFixedSize(output_shape,
                                                  output_type)
            interpreter.run(input, output.getBuffer().rewind())
...
#                         output.arg_ref), c_float), self.output_shape)
    ...
    class TensorFlowModel:
        ...
        def resize_input(shape):
            if list(get_input_shape()) != shape:
                interpreter.resize_tensor_input(0, shape)
                interpreter.allocate_tensors()
